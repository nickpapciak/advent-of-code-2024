...
def __():
    ...
    def parse_data(old_data): 
        old_data = [d.splitlines() for d in old_data.split("\n\n")]

        data = []
        for machine in old_data: 
            _, ax, ay = machine[0].split('+')
            ax, ay = int(ax.split(',')[0]), int(ay)

            _, bx, by = machine[1].split('+')
            bx, by = int(bx.split(',')[0]), int(by)
            
            _, prize_x, prize_y = machine[2].split('=')
            prize_x, prize_y = int(prize_x.split(',')[0]), int(prize_y)
            
            data.append(Machine(ax, ay, bx, by, prize_x, prize_y))
        return data
        
    data = parse_data(data)
    return Machine, data, dataclass, parse_data
...
def __(data):
    ...
    def solve_machine(m): 
        """basically does matrix inverse to see if possible to mash buttons
        """
        ...
        num_a = det*(m.by*m.prize_x - m.bx*m.prize_y)
        num_b = det*(m.ax*m.prize_y - m.ay*m.prize_x)
        tolerance = 1e-4
        is_valid = (abs(num_a - round(num_a)) < tolerance 
                and abs(num_b - round(num_b)) < tolerance 
               )
        
        return (round(num_a), round(num_b)) if is_valid else None

    def cost_of_machine(m): 
        soln = solve_machine(m)
        if soln is None: return 0
        return soln[0]*3 + soln[1]

    def cost_of_corrected_machine(m): 
        new_m = deepcopy(m)
        new_m.prize_x += 1e13
        new_m.prize_y += 1e13
        soln = solve_machine(new_m)
        if soln is None: return 0
        return soln[0]*3 + soln[1]

    print(sum(cost_of_machine(m) for m in data)) # 26810
    print(sum(cost_of_corrected_machine(m) for m in data)) # 108713182988244
    return (
        cost_of_corrected_machine,
        cost_of_machine,
        deepcopy,
        solve_machine,
    )
